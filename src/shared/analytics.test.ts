...
// ---------- origin 维度(Windows/WSL) ----------

describe('origin dimension', () => {
  /** 构造:同一天里 windows 与 wsl 两侧各有一个同名 claude + 一个 wsl 独有 zcode */
  const day = {
    date: '2026-08-21',
    inputTokens: 30, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0,
    totalTokens: 30, totalCost: 0,
    agents: [
      {
        agent: 'claude', inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0,
        totalTokens: 10, totalCost: 0,
        models: [{ model: 'm1', inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0, totalTokens: 10, totalCost: 0 }]
      },
      {
        agent: 'claude', origin: 'wsl' as const, inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0,
        totalTokens: 10, totalCost: 0,
        models: [{ model: 'm1', inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0, totalTokens: 10, totalCost: 0 }]
      },
      {
        agent: 'zcode', origin: 'wsl' as const, inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0,
        totalTokens: 10, totalCost: 0,
        models: [{ model: 'glm', inputTokens: 10, outputTokens: 0, cacheReadTokens: 0, cacheCreationTokens: 0, totalTokens: 10, totalCost: 0 }]
      }
    ]
  }

  it('aggregateAgents keeps same-name agents separate across origins', () => {
    const agents = aggregateAgents([day])
    expect(agents).toHaveLength(3)
    const keys = agents.map((a) => `${a.agent}@${a.origin ?? 'windows'}`).sort()
    expect(keys).toEqual(['claude@windows', 'claude@wsl', 'zcode@wsl'])
  })

  it('filterByOrigin keeps only the target side and recomputes day totals', () => {
    const wslOnly = filterByOrigin([day], 'wsl')
    expect(wslOnly[0].agents).toHaveLength(2)
    expect(wslOnly[0].totalTokens).toBe(20)

    const winOnly = filterByOrigin([day], 'windows')
    expect(winOnly[0].agents).toHaveLength(1)
    expect(winOnly[0].agents[0].agent).toBe('claude')
    expect(winOnly[0].totalTokens).toBe(10)

    const all = filterByOrigin([day], 'all')
    expect(all[0]).toBe(day)
  })

  it('aggregateModels splits Used By rows per (agent, origin)', () => {
    const models = aggregateModels([day])
    const m1 = models.find((m) => m.model === 'm1')!
    expect(m1.totalTokens).toBe(20)
    expect(m1.agents).toHaveLength(2)
    const shares = m1.agents.map((a) => `${a.agent}@${a.origin ?? 'windows'}`).sort()
    expect(shares).toEqual(['claude@windows', 'claude@wsl'])
  })

  it('aggregateAgentModels matches the agentKey including origin', () => {
    const winClaude = aggregateAgentModels([day], 'claude')
    expect(winClaude).toHaveLength(1)
    expect(winClaude[0].totalTokens).toBe(10)

    const wslClaude = aggregateAgentModels([day], 'claude@wsl')
    expect(wslClaude).toHaveLength(1)
    expect(wslClaude[0].totalTokens).toBe(10)

    const wslZcode = aggregateAgentModels([day], 'zcode@wsl')
    expect(wslZcode[0].model).toBe('glm')
  })

  it('shareDrift agent series are keyed by agentKey', () => {
    const drift = shareDrift([day], 'agents', 5)
    expect(drift.names.sort()).toEqual(['claude', 'claude@wsl', 'zcode@wsl'])
  })
})
