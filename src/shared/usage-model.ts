...
export type { UsageOrigin } from './agents'
...
/** 合并已规范化的 AgentUsage;同名 agent 跨 origin(Windows/WSL)各自独立条目 */
export function mergeNormalizedAgent(agents: Map<string, AgentUsage>, incoming: AgentUsage): void {
  const key = agentKeyOf(incoming)
  const existing = agents.get(key)
  if (!existing) {
    agents.set(key, { ...incoming, models: incoming.models.map((m) => ({ ...m })) })
    return
  }
...
export function mergeDailyIntoSnapshot(
  ...
  for (const day of extra) {
    ...
    const agents = new Map(existing.agents.map((a) => [agentKeyOf(a), a]))
    for (const a of day.agents) {
      mergeNormalizedAgent(agents, a)
    }
...
export function rowsToDaily<R>(
  ...
  for (const row of rows) {
    ...
    const agents = new Map(day.agents.map((a) => [agentKeyOf(a), a]))
