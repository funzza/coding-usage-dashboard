...
/** 跨 agent 聚合 model 维度,并记录每个模型在各 agent 下的用量 */
export function aggregateModels(daily: DailyUsage[]): ModelAggregate[] {
  const models = new Map<string, ModelAggregate & { _agents: Map<string, TokenUsage> }>()
  for (const day of daily) {
    for (const agent of day.agents) {
      for (const m of agent.models) {
        ...
        addUsage(entry, m)
        const share = entry._agents.get(agent.agent) ?? zeroUsage()
        addUsage(share, m)
        entry._agents.set(agent.agent, share)
      }
    }
  }
...
/** 单个 agent 在给定 daily 内的 model 聚合 */
export function aggregateAgentModels(daily: DailyUsage[], agentName: string): ModelUsage[] {
  const models = new Map<string, ModelUsage>()
  for (const day of daily) {
    const entry = day.agents.find((a) => a.agent === agentName)
    if (!entry) continue
...
export function shareDrift(
  ...
  const perDay = filledDaily.map((day) => {
    ...
    if (mode === 'agents') {
      for (const a of day.agents) m.set(a.agent, (m.get(a.agent) ?? 0) + a.totalTokens)
    } else {
      for (const a of day.agents) {
...
export function sessionsHourlyBuckets(sessions: SessionUsage[], date: string): HourBucket[] {
  ...
  for (const s of sessions) {
    ...
    b.agents[s.agent] = (b.agents[s.agent] ?? 0) + s.totalTokens
