...
import { agentColor, displayAgentKey, seriesColor } from '../utils/agent'
...
const option = computed(() => {
  ...
  return {
    ...
    series: names.map((name, i) => {
      const color = props.mode === 'agents' ? agentColor(name) : seriesColor(name)
      return {
        // agents 模式的系列身份是 agentKey(@wsl 后缀),显示名剥后缀加 (WSL)
        name: props.mode === 'agents' ? displayAgentKey(name) : name,
        type: 'line',
        stack: 'share',
        symbol: 'none',
        smooth: false,
