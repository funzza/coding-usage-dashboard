...
import { compositionTooltip, topWithOther, type RankItem } from '../utils/composition'
...
const agent = computed(() => aggregateAgents(rangedDaily.value).find((a) => a.agent === props.name))
...
const milestones = computed(() => {
  ...
  const totals = store.snapshot.daily
    ...
    .map((d) => d.agents.find((a) => a.agent === props.name)?.totalTokens ?? 0)
  return milestonesOf(totals)
})

/** Today:donut 条目 = 该 agent 的 models(top 10 + Other) */
const donutItems = computed<RankItem[]>(() =>
  topWithOther(models.value.map((m) => ({ name: m.model, color: seriesColor(m.model), usage: m })))
)
  ...
  <div v-if="agent" class="page">
    <header class="head drag-head">
      <div class="title">
        ...
        <h1>{{ displayAgentName(agent.agent) }}</h1>
      </div>
      <RangeTabs />
    </header>
    ...
    <template v-else>
      <section class="panel">
        ...
        <TrendChart :days="filledDaily" mode="agent" :name="agent.agent" :range="store.range" />
      </section>
...
.row {
  display: grid;
  grid-template-columns: 10px 260px 1fr 80px 70px 56px;
  align-items: center;
  gap: 12px;
  padding: 7px 0;
