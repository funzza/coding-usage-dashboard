...
import OriginSwitch from '../components/OriginSwitch.vue'
...
/** 全局 origin 筛选(Windows/WSL)在 range 过滤前先裁掉非目标侧的 agent */
const originDaily = computed(() =>
  snapshot.value ? filterByOrigin(snapshot.value.daily, store.origin) : []
)
const rangedDaily = computed(() => filterDailyByRange(originDaily.value, store.range))
const filledDaily = computed(() => selectRangeDaily(originDaily.value, store.range))
...
const milestones = computed(() =>
  snapshot.value && isToday.value ? usageMilestones(originDaily.value, todayIso()) : null
)

/** 区间统计(仅非 today) */
const stats = computed(() =>
  snapshot.value && !isToday.value ? rangeStats(originDaily.value, store.range) : null
)
...
/** Today 活动图:session 数据按小时分桶(仅 today 态懒加载),受 origin 筛选 */
const hourlyBuckets = computed(() => {
  const sessions = (sessionsStore.report?.sessions ?? []).filter(
    (s) => store.origin === 'all' || (s.origin ?? 'windows') === store.origin
  )
  return sessionsHourlyBuckets(sessions, todayIso())
})
...
/** donut 条目:与明细列表同源,不聚合——所有模型/agent 各自一片 */
const donutItems = computed<RankItem[]>(() => {
  ...
  return models.value.map((m) => ({ name: m.model, color: seriesColor(m.model), usage: m }))
})
