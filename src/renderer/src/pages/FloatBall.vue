...
const config = ref<FloatConfig>({ size: 'm', opacity: 1, animation: 'lively', colorMode: 'adaptive' })
  ...
  >
    <section v-if="expanded" class="panel" :style="{ height: `calc(100% - ${ballSize + 8}px)` }">
      ...
      <ul class="agent-list" v-if="todayAgents.length > 0">
        <li v-for="agent in todayAgents" :key="agent.agent">
      ...
      <div v-if="quotaRows.length > 0" class="quota-block">
        <div class="quota-row" v-for="r in quotaRows" :key="r.quota.accountId" :title="r.detail">
          <span class="dot" :style="{ background: agentColor(r.quota.agent) }" />
          <span class="quota-name">{{ r.quota.label }}</span>
    ...
    <BallVisual
      ...
      :title="`Today ${formatTokens(todayTotal)}${quotaAlarm ? ` · ${quotaAlarm.windowLabel} ${Math.round(quotaAlarm.usedPercent)}%` : ''}`"
