...
import { agentColor, agentKeyOf, displayAgentKey } from '../utils/agent'
...
/** agent 筛选候选 = agentKey(同名 agent 的 Windows/WSL 两侧各自一行) */
const agents = computed(() => {
  const set = new Set<string>()
  for (const s of report.value?.sessions ?? []) {
    set.add(agentKeyOf(s))
  }
  return [...set].sort()
})

const filtered = computed<SessionUsage[]>(() => {
  let sessions = report.value?.sessions ?? []
  if (usageStore.origin !== 'all') {
    sessions = sessions.filter((s) => (s.origin ?? 'windows') === usageStore.origin)
  }
  if (filterAgent.value !== 'all') {
    sessions = sessions.filter((s) => agentKeyOf(s) === filterAgent.value)
  }
  return sessions
})
    ...
    <header class="head drag-head">
      ...
      <div class="actions">
        <select v-if="agents.length > 1" v-model="filterAgent" class="filter">
          ...
          <option v-for="a in agents" :key="a" :value="a">{{ displayAgentKey(a) }}</option>
        </select>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="s in filtered" :key="s.id">
            ...
            <td>
              <span class="agent">
                <span class="dot" :style="{ background: agentColor(agentKeyOf(s)) }" />
                {{ displayAgentKey(agentKeyOf(s)) }}
              </span>
            </td>
