...
import { filterByOrigin } from '../../shared/analytics'
import type { AgentUsage } from '../../shared/usage-model'
...
/** 侧边 Today 速览的维度与 Overview 共用 store.dimension(默认 Models);受全局 origin 筛选 */
const todayAgents = computed(() => {
  const last = store.snapshot?.daily[store.snapshot.daily.length - 1]
  return last ? filterByOrigin([last], store.origin)[0].agents : []
})

/** 侧边栏快速看板固定为 Today 语义:显示值与排序一致(today tokens 降序) */
const sidebarAgents = computed(() => {
  const filtered = store.agents.filter(
    (a) => store.origin === 'all' || (a.origin ?? 'windows') === store.origin
  )
  return [...filtered].sort((a, b) => todayValue(b) - todayValue(a))
})
...
function todayValue(agent: AgentUsage): number {
  const key = agentKeyOf(agent)
  return todayAgents.value.find((a) => agentKeyOf(a) === key)?.totalTokens ?? 0
}
...
function todayTokens(agent: AgentUsage): string {
  ...
  <div v-else class="layout">
    ...
    <section class="content">
      ...
      <footer class="statusbar">
        ...
        <span>{{ sidebarAgents.length }} AGENTS</span>
