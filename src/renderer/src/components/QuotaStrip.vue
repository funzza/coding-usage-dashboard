...
function color(q: QuotaAccount): string {
  return agentColor(quotaAgentKey(q.agent, q.origin))
}
...
<template>
  <nav v-if="sorted.length > 0" class="strip" aria-label="Subscriptions">
    <router-link
      ...
      class="item"
      to="/subscriptions"
      :title="`${q.label}${q.origin === 'wsl' ? ' (WSL)' : ''} — open Subscriptions`"
    >
      <span class="dot" :style="{ background: color(q) }" />
      <span class="label">{{ q.label }}<i v-if="q.origin === 'wsl'" class="origin-tag">W</i></span>
