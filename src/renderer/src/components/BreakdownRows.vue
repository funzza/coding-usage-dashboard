...
<template>
  <template v-if="!isEmpty">
    ...
    <ul v-if="dimension === 'agents'" class="rows">
      <li v-for="a in agents" :key="a.agent">
        <router-link :to="`/agents/${a.agent}`" class="row" :title="compositionTooltip(a)">
          <span class="dot" :style="{ background: agentColor(a.agent) }" />
          <span class="name">{{ displayAgentName(a.agent) }}</span>
...
.row {
  display: grid;
  grid-template-columns: 10px 140px 1fr 80px 70px 52px 56px;
  align-items: center;
  gap: 12px;
  padding: 7px 0;
...
.row.model {
  grid-template-columns: 10px 240px 1fr 80px 70px 52px 56px;
}
