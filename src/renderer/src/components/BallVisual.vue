...
import type { FloatAnimation } from '../../../shared/float-config'
import { QUOTA_ALARM_COLORS, type QuotaAlarmLevel } from '../utils/ball'

/**
 * 悬浮球球体纯展示组件(FloatBall 页面与 Settings 预览共用):
 * - SVG 圆环:填充比例 = 今日总量 / P90(由父组件算好传入),轨道半透明灰,填充弧用档位色
 * - 动画语言(克制,置顶窗口不能吵):
...
const props = defineProps<{
  ...
  /** 刷新失败信息;变为非空时球体轻抖一下 */
  error?: string
  /** quota 警示档(>=90% 琥珀 / >=98% 红);null/缺省不显示警示点 */
...
const dashOffset = computed(() => CIRCUMFERENCE * (1 - Math.min(1, Math.max(0, props.fillRatio))))

const arcStroke = computed(() =>
  props.tier === 'blazing' ? `url(#${gradientId})` : TIER_COLORS[props.tier]
)
const glowColor = computed(() =>
  props.tier === 'blazing' ? '#ff8f5e' : TIER_COLORS[props.tier]
)
...
<template>
  <div
    class="ball-visual"
    :class="[animation, { refreshing, shaking }]"
    ...
    <div class="hover-zoom">
      <div class="breathe">
        <div class="float-y">
          ...
          <span
            class="value"
            :class="{ blazing: tier === 'blazing' }"
            :style="{ fontSize: `${fontSize}px`, color: glowColor }"
          >{{ valueText }}</span>
          <!-- quota 警示点:右上角呼吸圆点;发光用 box-shadow,只作用于这个圆形元素,
