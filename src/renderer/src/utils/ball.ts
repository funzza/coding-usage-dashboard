...
/**
 * 环形悬浮球(ring 形态)的取数:扫所有账号所有窗口,取消耗百分比最高者,
 * 不设告警阈值(环永远显示当前最紧张的配额状态;无任何窗口数据时返回 null)。
 */
export interface QuotaRing {
  percent: number
  windowLabel: string
  accountLabel: string
}

export function resolveQuotaRing(accounts: QuotaAccount[]): QuotaRing | null {
  let worst: QuotaRing | null = null
  for (const account of accounts) {
    for (const w of account.windows) {
      if (w.usedPercent > (worst?.percent ?? -1)) {
        worst = {
...
/** 配额环三档配色:与展开面板的配额进度条同色(绿 <60 / 黄 <85 / 红 ≥85) */
export function quotaRingColor(percent: number): string {
