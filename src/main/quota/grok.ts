...
export async function collectGrokQuota(cred: QuotaCredential): Promise<{
  ...
  // 订阅档位只存在于本机日志;manual 账号(可能在别的机器)拿不到。身份同理(local 的 user_id)
  return {
    ...parsed,
    plan: cred.source === 'local' ? readSubscriptionTier() : null,
    remoteUserId: cred.source === 'local' ? creds.userId : null
  }
}
