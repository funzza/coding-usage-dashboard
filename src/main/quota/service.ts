...
interface ProviderDef {
  ...
  localCredentialExists: () => boolean
  collect: (cred: QuotaCredential) => Promise<{
...
const PROVIDERS: ProviderDef[] = [
  { id: 'kimi', agent: 'kimi', displayName: 'Kimi', supportsManual: false, localCredentialExists: kimiCredentialExists, collect: () => collectKimiQuota() },
  { id: 'codex', agent: 'codex', displayName: 'ChatGPT', supportsManual: true, localCredentialExists: codexCredentialExists, collect: collectCodexQuota },
  { id: 'opencode-go', agent: 'opencode', displayName: 'OpenCode Go', supportsManual: true, localCredentialExists: opencodeGoCredentialExists, collect: collectOpencodeGoQuota },
  { id: 'grok', agent: 'grok', displayName: 'Grok', supportsManual: true, localCredentialExists: grokCredentialExists, collect: collectGrokQuota }
]
...
function resolveCredential(account: QuotaAccountConfig): QuotaCredential | null {
  if (account.source === 'local') {
    return providerDef(account.provider).localCredentialExists() ? { source: 'local' } : null
  }
...
async function collectAccount(
  ...
  try {
    const result = await def.collect(cred)
    return {
      ...base,
...
/** 应用启动后调用:立刻采一次(不阻塞 ready),之后 120s 轮询 */
...
/** before-quit:停轮询,杀掉我们拉起的 kimi web sidecar(用户已有实例不碰) */
